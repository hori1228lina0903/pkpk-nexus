...
generateCardsHTML(cards) {
    ...
    for (let i = 0; i < cards.length; i++) {
        ...
        const cardName = card['名前'] || 'Unknown Card';
        const cacheBuster = `?t=${now}`;
        
        if (i < preloadCount) {
            // 最初のバッチの最初の16枚だけ先行読み込み
            cardsHTML.push(`
                <a href="#" class="card" data-card-name="${cardName}">
    ...
    setupFilterButtons() {
        ...
        filterButtons.forEach(button => {
            button.addEventListener('click', (e) => {
                ...
                console.log('クリックされた要素のタグ名:', e.target.tagName);
                ...
                if (e.target.tagName === 'IMG' || 
                    ...
                    console.log('親ボタン要素:', targetButton);
                } else {
                ...
                if (!targetButton || !targetButton.classList.contains('filter-btn')) {
                    console.error('エラー: ボタン要素を正しく取得できませんでした');
                    return;
                }
                ...
                console.log('取得したフィルター情報 - タイプ:', filterType, '値:', filterValue);
                console.log('現在のactiveクラス状態:', targetButton.classList.contains('active'));
                
                this.toggleFilterButton(targetButton, filterType, filterValue);
                this.performFilteredSearch();
                console.log('=== クリックイベント終了 ===');
            });
        });
    }
...
syncHpOptions() {
    ...
    toggleFilterButton(button, filterType, filterValue) {
        console.log('toggleFilterButton開始');
        console.log('現在のフィルター状態:', JSON.parse(JSON.stringify(this.currentFilters)));
        
        // カードの種類フィルターの特別処理（単一選択）
        if (filterType === 'cardType') {
            console.log('カードの種類フィルターを処理:', filterValue);
            
            // すべてのカードの種類ボタンのactiveクラスを削除
            ...
            if (filterValue === 'all') {
                ...
                console.log('すべてのカードを表示するように設定');
            } else {
                this.currentFilters.cardType = [filterValue];
                console.log('カードの種類フィルターを設定:', filterValue);
            }
        } else {
            // その他のフィルターは従来通り複数選択
            const currentIndex = this.currentFilters[filterType].indexOf(filterValue);
            console.log('filterValueのインデックス:', currentIndex);
            
            if (currentIndex > -1) {
                console.log('フィルターを解除します');
                this.currentFilters[filterType].splice(currentIndex, 1);
                button.classList.remove('active');
                console.log('activeクラスを削除');
            } else {
                console.log('フィルターを追加します');
                this.currentFilters[filterType].push(filterValue);
                button.classList.add('active');
                console.log('activeクラスを追加');
            }
        }
        
        console.log('更新後のフィルター状態:', JSON.parse(JSON.stringify(this.currentFilters)));
        console.log('更新後のactiveクラス状態:', button.classList.contains('active'));
        console.log('toggleFilterButton終了');
...
resetSearch() {
    console.log('🔘 resetSearch ボタン押下');
    
    const searchInput = document.getElementById('cardSearch');
    if (searchInput) {
        console.log('🗑️ 検索テキストをクリア');
        searchInput.value = '';
    }
    ...
    // 新しい検索IDを生成して即時実行
    const searchId = ++this.currentSearchId;
    console.log(`🔄 resetSearch: 検索ID ${searchId} で即時検索実行`);
...
handleSearch(searchTerm, searchId = 0) {
    ...
    if (searchId < this.currentSearchId) {
        console.log(`⏭️ 検索ID ${searchId} は古いのでスキップ (最新: ${this.currentSearchId})`);
        return;
    }
    ...
    console.log('   フィルター状態:', JSON.stringify(this.currentFilters));
...
resetAdvancedFilters() {
    ...
    const advancedFilters = document.getElementById('advancedFilters');
    if (advancedFilters) {
        console.log('📌 advancedFiltersコンテナ内のボタンをリセット');
        ...
        console.log('⚠️ advancedFiltersコンテナが見つかりません');
...
applyPackFilter() {
    ...
    console.log('   現在のフィルター状態:', JSON.stringify(this.currentFilters.pack));
    console.log('   フィルター配列の長さ:', this.currentFilters.pack.length);
    ...
    setupPackSelectEvents() {
        ...
        console.log(`🔧 ${selectItemElements.length}個のオプション要素にイベント設定`);
    
        selectItemElements.forEach(item => {
            item.addEventListener('click', (e) => {
                e.stopPropagation();
                console.log('🎯 オプションクリック:', item.dataset.value);
    ...
    applyPackFilter(selectedValue) {
        ...
        console.log('拡張パックフィルターを適用:', this.currentFilters.pack);
        this.performFilteredSearch();
    }
...
async showAllCards() {
    console.log('🚨🚨🚨 showAllCards() が呼ばれました！');
    ...
    console.log('👤 呼び出し元:', callerLine.trim());
    
    // 現在の状態をログ
    console.log('📊 現在の状態:');
    console.log('  - currentSearchId:', this.currentSearchId);
    console.log('  - searchTimer:', this.searchTimer);
    console.log('  - フィルター状態:', JSON.stringify(this.currentFilters));
    ...
    // データがない場合は表示しない
    if (!this.cardData || this.cardData.length === 0) {
        console.log('⚠️ 表示するカードデータがありません');
        return;
    }
    
    console.log('🔍 全てのカードを表示:', this.cardData.length, '枚');
    ...
    setTimeout(async () => {
        ...
        const cardsHTML = await Promise.all(this.cardData.map(async (card, index) => {
            ...
            return new Promise((resolve) => {
                ...
                testImg.onload = () => {
                    // 画像が正常に読み込めた場合
                    console.log(`✅ ${index + 1}/${this.cardData.length}: ${card['名前']} - 読み込み成功`);
                    resolve(`
                        <a href="#" class="card">
                            <div class="card-image">
        ...
        if (!currentResultsContainer) {
            console.log('⚠️ 結果コンテナが削除されました');
            return;
        }
        
        // すべての画像HTMLが準備できたら表示を更新
        currentResultsContainer.innerHTML = cardsHTML.join('');
        console.log('✅ 全てのカード表示完了');
        
    }, 100); // 100ms待機してDOMの準備を確実にする
}

async showSearchResults(cards, searchId = null) {
    console.log('🎯 showSearchResults 開始');
    console.log(`   表示するカード数: ${cards.length}件`);
    console.log(`   全カード数: ${this.cardData.length}件`);
...
restoreFilterState() {
    // このメソッドは localStorage を使用しないシンプルな実装
    console.log('🔧 restoreFilterState() 実行');
    ...
    getCardImagePath(card) {
        try {
            ...
            if (this.failedImages.has(cacheKey)) {
                console.log(`🔄 ${card['名前']}: 以前失敗した画像 → デフォルトを使用`);
                return this.getDefaultImagePath();
            }
            
            if (!card || typeof card !== 'object') {
                console.warn('❌ 無効なカードオブジェクト');
                return this.getDefaultImagePath();
            }
...
resetFilters() {
    ...
    // showAllCards()は呼ばない - resetSearch()から呼ばれる
}
